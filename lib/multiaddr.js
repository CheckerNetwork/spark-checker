...
export function multiaddrToHttpUrl (addr) {
  ...
  if (addr.includes('/http-path')) {
    ...
    } // Handle DNS-based direct http/https format
  } else if (hostType.startsWith('dns') && (multiAddrParts[0] === 'http' || multiAddrParts[0] === 'https')) {
    scheme = multiAddrParts[0]
    rest = multiAddrParts.slice(1)
  } else {
    let ipProtocol
