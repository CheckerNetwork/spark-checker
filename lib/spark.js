...
export default class Spark {
  ...
  async executeRetrievalCheck(retrieval, stats) {
    ...
    if (noValidAdvertisement) {
      ...
      stats.alternativeProviderRetrieval = await this.checkRetrievalFromAlternativeProvider(alternativeProviders, retrieval.cid)
      return
    }
  ...
  async checkRetrievalFromAlternativeProvider(providers, cid) {
    if (!providers.length) {
    ...
    const alternativeProviderRetrievalStats = newAlternativeProviderRetrievalStats()
    await this.fetchCAR(
...
export function newStats() {
  return {
    ...
    statusCode: null,
    headStatusCode: null,
    alternativeProviderRetrieval: null,
  }
}

function newAlternativeProviderRetrievalStats() {
  return {
    statusCode: null,
    timeout: false,
    endAt: null,
...
/**
 * TODO: Use pseudo-random generator to pick a provider
 * 
 * Picks a provider based on the following priority:
 * 1. Providers with ContextID starting with 'ghsA' and supporting HTTP.
 * 2. Providers with ContextID starting with 'ghsA' and supporting Graphsync.
 * 3. Providers supporting HTTP.
 * 4. Providers supporting Graphsync.
 *
...
export function pickRandomProvider(providers) {
  ...
  const providersWithContextID = providers.filter((p) => p.contextId.startsWith('ghsA') && p.protocol !== 'bitswap')

  // Priority 1: HTTP providers with ContextID containing PieceCID
  const ghsAHttp = filterByProtocol(providersWithContextID, 'http')
  if (ghsAHttp.length) return pickRandomItem(ghsAHttp)

  // Priority 2: Graphsync providers with ContextID containing PieceCID
  const ghsAGraphsync = filterByProtocol(providersWithContextID, 'graphsync')
  if (ghsAGraphsync.length) return pickRandomItem(ghsAGraphsync)

  // Priority 3: HTTP providers
  const httpProviders = filterByProtocol(providers, 'http')
  if (httpProviders.length) return pickRandomItem(httpProviders)

  // Priority 4: Graphsync providers
  const graphsyncProviders = filterByProtocol(providers, 'graphsync')
  if (graphsyncProviders.length) return pickRandomItem(graphsyncProviders)
