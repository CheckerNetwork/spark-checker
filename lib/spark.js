...
export default class Spark {
  ...
  async executeRetrievalCheck(retrieval, stats) {
    ...
    if (noValidAdvertisement) {
      ...
      return await this.testNetworkRetrieval(providers, retrieval, stats)
    }
  ...
  async testNetworkRetrieval(providers, retrieval, stats) {
    if (!providers.length) {
      console.warn('No providers found for the CID. Skipping network-wide retrieval check.')
      return
    }

    const networkRetrievalStats = { statusCode: null, timeout: false }
    ...
    stats.networkRetrievalProtocol = randomProvider.protocol
    stats.networkRetrievalTimeout = networkRetrievalStats.timeout
    stats.networkRetrievalStatusCode = networkRetrievalStats.statusCode
    stats.networkRetrievalCarTooLarge = networkRetrievalStats.carTooLarge
    stats.networkRetrievalEndAt = networkRetrievalStats.endAt
  }
...
function pickRandomProvider(providers) {
  const { sum, weights } = weighProviders(providers)
  return providers[pickRandomWeightedIndex(sum, weights)]
}
 ...
 *
 * @param {Provider[]} providers
 * @returns {{ sum: number; weights: number[] }}
 */
 ...
 * @returns
 */
function pickRandomWeightedIndex(sum, weights) {
  ...
  while (low < high) {
    ...
    if (index === weights[mid]) return mid
