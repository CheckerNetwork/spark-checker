...
export default class Spark {
  ...
  async executeRetrievalCheck(retrieval, stats) {
    ...
    if (noValidAdvertisement) {
      ...
      stats.networkRetrieval = await this.checkRetrievalFromAlternativeProvider(alternativeProviders, retrieval.cid)
      return
    }
  ...
  async checkRetrievalFromAlternativeProvider(providers, cid) {
    ...
    const networkRetrievalStats = newNetworkRetrievalStats()
    await this.fetchCAR(
      randomProvider.protocol,
      randomProvider.address,
      cid,
      networkRetrievalStats,
    )

    return networkRetrievalStats
  }
...
export function pickRandomProvider(providers) {
  ...
  if (ghsAHttp.length) return randomItem(ghsAHttp)
  ...
  if (ghsAGraphsync.length) return randomItem(ghsAGraphsync)

  // Priority 3: HTTP providers
  const httpProviders = filterByProtocol(providers, 'http')
  if (httpProviders.length) return randomItem(httpProviders)

  // Priority 4: Graphsync providers
  const graphsyncProviders = filterByProtocol(providers, 'graphsync')
  if (graphsyncProviders.length) return randomItem(graphsyncProviders)
