...
function random(randomness) {
  ...
  randomness = (randomness * 48271) % 2147483647
  return randomness / 2147483647
}

function pickRandomItem(items, randomness) {
  if (!items.length) return undefined
  const index = Math.floor(random(randomness) * items.length)
  return items[index]
}
