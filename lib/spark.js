...
export default class Spark {
  ...
  async executeRetrievalCheck(retrieval, stats) {
    ...
    if (noValidAdvertisement) {
      ...
      return await this.testNetworkWideRetrieval(providers, retrieval, stats)
    }
  ...
  async testNetworkWideRetrieval(providers, retrieval, stats) {
    if (!providers.length) {
      console.warn('No providers found for the CID. Skipping network-wide retrieval check.')
      return
    }

    const networkWideRetrievalStats = { statusCode: null, timeout: false }
    const randomProvider = pickRandomProvider(providers)
    await this.fetchCAR(
      randomProvider.protocol,
      randomProvider.address,
      retrieval.cid,
      networkWideRetrievalStats,
    )

    stats.networkWideRetrievalTimeout = networkWideRetrievalStats.timeout
    stats.networkWideRetrievalStatusCode = networkWideRetrievalStats.statusCode
  }
 ...
 */
function pickRandomProvider(/** @type {} */ providers) {
