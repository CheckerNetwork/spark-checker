...
export async function getMinerPeerId (minerId, { maxAttempts = 5 } = {}) {
  // Make a concurrent request to both sources: RPC node and smart contract
  const [rpcNodeResult, contractResult] = await Promise.allSettled([
  ...
  // Fall back to rpc-Node result
  if (rpcNodeResult.status === 'fulfilled' && rpcNodeResult.value) {
    return rpcNodeResult.value
  }

  // Handle the case where both failed
  throw new Error(`Failed to resolve PeerID from any source: ${contractResult.reason}, ${rpcNodeResult.reason}`)
}
