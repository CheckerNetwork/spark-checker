...
export async function getMinerPeerId (minerId, { maxAttempts = 5 } = {}) {
  ...
  const [onChainResult, contractResult] = await Promise.allSettled([
  ...
  // Fall back to on-chain result
  if (onChainResult.status === 'fulfilled' && onChainResult.value) {
    return onChainResult.value
  }

  // Handle the case where both failed
  throw new Error(`Failed to resolve PeerID from any source: ${contractResult.reason}, ${onChainResult.reason}`)
}
